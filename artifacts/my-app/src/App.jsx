...
const savePlan=useCallback(plan=>{const key=(plan.stops||[]).map(v=>String(v.id)).sort().join(",");setSavedPlans(prev=>{if(prev.some(p=>(p.stops||[]).map(v=>String(v.id)).sort().join(",")===key))return prev;showToast("\u2728 Night saved");return [{...plan,id:Date.now()+"p",savedAt:new Date().toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"})},...prev.slice(0,19)];});},[showToast]);
