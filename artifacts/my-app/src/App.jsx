...
const goCategory=useCallback(c=>{setCat(c);setSection("explore");setTimeout(()=>{filtersRef.current?.scrollIntoView({behavior:"smooth",block:"start"});if(chipRowRef.current){const label=c==="all"?"All Spots":c;const btns=chipRowRef.current.querySelectorAll("button");for(const btn of btns){if(btn.textContent.trim()===label){const row=chipRowRef.current;const target=btn.offsetLeft-(row.clientWidth/2)+(btn.offsetWidth/2);row.scrollTo({left:Math.max(0,target),behavior:"smooth"});break;}}}},60);},[]);
